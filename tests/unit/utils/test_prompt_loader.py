...
""")
    ...
    def test_init_with_non_dict_content(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.yaml', delete=False) as f:
            ...
            try:
                with pytest.raises(ValueError, match="Prompts file must contain a dictionary"):
                    PromptLoader(f.name)
            finally:
                Path(f.name).unlink()
...
""")
            ...
            try:
                ...
                with pytest.raises(ValueError, match="Prompt 'invalid_prompt' is missing 'prompt' field"):
...
""")
            ...
            try:
                ...
                with pytest.raises(ValueError, match="Prompt 'invalid_prompt' value must be a string"):
                    loader.get_prompt("invalid_prompt")
            finally:
                Path(f.name).unlink()
...
""")
            ...
            try:
                ...
                assert loader.language_detection_prompt == "Detect language: {files_content}"
                ...
                assert loader.project_summary_prompt == "Summarize project: {total_files}"
            finally:
                Path(f.name).unlink()
    ...
    def test_property_methods_missing_prompts(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.yaml', delete=False) as f:
            ...
            try:
                ...
                with pytest.raises(KeyError, match="Prompt 'language_detection' not found"):
                    _ = loader.language_detection_prompt

                with pytest.raises(KeyError, match="Prompt 'single_file_analysis' not found"):
                ...
                with pytest.raises(KeyError, match="Prompt 'project_summary' not found"):

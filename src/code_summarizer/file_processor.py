...
class FileProcessor:
    def __init__(self, config_path: str = 'config.yaml'):
        ...
        self.supported_extensions = self.config.get('file_processing', {}).get('supported_extensions', [])
        self.exclude_patterns = self.config.get('file_processing', {}).get('exclude_patterns', [])
    ...
    def _scan_directory(self, directory: str) -> List[str]:
        ...
        for root, dirs, files in os.walk(directory):
            ...
            dirs[:] = [d for d in dirs if not any(pattern.replace('*', '') in os.path.join(root, d) 
                                                 for pattern in self.exclude_patterns)]
            
            for file in files:
                file_path = os.path.join(root, file)
                
                if self._is_supported_file(file) and not self._should_exclude(file_path):
                    code_files.append(file_path)
        
        return code_files
    
    def _create_file_data(self, file_paths: List[str], base_dir: str = None) -> List[Dict[str, Any]]:
    ...
    def get_project_info(self, files_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            'file_types': {lang: len([f for f in files_data if self._extension_to_language(f['extension']) == lang]) 

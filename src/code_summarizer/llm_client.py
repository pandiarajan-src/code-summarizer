...
class LLMClient:
    ...
    def _make_api_call(self, prompt: str) -> str:
        ...
        try:
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are a code analysis expert. Always respond with valid JSON when requested."},
    ...
    def _parse_json_response(self, response: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise Exception(f"Could not parse JSON from LLM response: {response[:200]}...")
    ...
    def detect_languages(self, files_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for file_data in files_data[:5]:  # Limit to first 5 files for detection
            files_summary.append(f"File: {file_data['name']}\n{file_data['content'][:500]}...\n")
...
"""
    ...
    def generate_project_summary(self, files_data: List[Dict[str, Any]], 
        ...
        analysis_summary = {
            ...
            'total_files_analyzed': sum(len(result.get('files', [])) for result in analysis_results),
            'key_findings': []
        }
        ...
        for result in analysis_results:
            if 'batch_summary' in result:
                analysis_summary['key_findings'].append(result['batch_summary'].get('main_purpose', ''))

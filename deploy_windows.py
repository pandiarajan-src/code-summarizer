...
def setup_env_file():
    ...
    with open(env_path, 'r') as f:
        ...
        if 'your_api_key_here' in content or 'OPENAI_API_KEY=' not in content:
            print_colored("⚠️  WARNING: OPENAI_API_KEY may not be set properly", Colors.YELLOW)
            if platform.system() == "Windows":
...
def install_dependencies():
    ...
    if shutil.which('uv'):
        ...
        result = subprocess.run('uv sync --frozen --no-dev', shell=True)
    ...
    core_deps = [
        ...
        'openai', 'tiktoken', 'click', 'python-multipart', 'pyyaml'
    ]
...
def main():
    ...
    # Check Python
    if not check_python():
        print_colored("Please install Python 3.12+ from https://www.python.org/", Colors.RED)
        sys.exit(1)
